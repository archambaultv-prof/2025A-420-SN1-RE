...
nommées et décrites. Notez qu'elles ne prennent aucun paramètre.
...
Créer un DataFrame contenant toutes les simulations (scénarios A, B, C et leurs versions avec bruit).
...
C'est-à-dire, chaque ligne du DataFrame correspond à une semaine d’un scénario
donné. Il faut donc concaténer les résultats des 6 simulations "verticalement".
...
[Grille d'évaluation](pathname:///files/420-SN1-TP2-Grille.xlsx)
...
2. Évolution temporelle par scénario (courbes A, B, C et A’, B’, C’ —
   température finale du lac, semaines 1 à 16).
3. La variation des températures finales du lac d'une semaine à l'autre
   (différence entre semaines consécutives) pour chaque scénario.
